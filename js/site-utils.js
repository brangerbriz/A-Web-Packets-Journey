...
class Didactic3JS {
    constructor(config){
        ...
        this.camera = new THREE.PerspectiveCamera(
            75, this.w/this.h, 0.1, 100)
        this.camera.position.x = config.cam.x
        this.camera.position.y = config.cam.y
        this.camera.position.z = config.cam.z
    ...
    captionPosition(){
        let cap = this.parent.querySelector('.caption-3js')
        if(cap){
            let cw = cap.offsetWidth
            let pw = this.parent.offsetWidth
            cap.style.left = `${pw/2 - cw/2}px`
        }
    }
    resize(){
        this.w = this.parent.offsetWidth
